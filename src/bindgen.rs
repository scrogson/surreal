//! Generate .surt type stubs from Erlang and Elixir source files.
//!
...
pub fn cmd_bindgen(files: &[std::path::PathBuf], output: Option<&Path>, _module: Option<&str>) -> ExitCode {
    ...
    // Generate output
    let output_content = generate_surt(&all_modules);
...
/// Generate .surt output from parsed module info.
fn generate_surt(modules: &HashMap<String, ModuleInfo>) -> String {
...
mod tests {
    ...
    fn test_generate_function_name_attribute() {
        ...
        let output = generate_surt(&modules);
